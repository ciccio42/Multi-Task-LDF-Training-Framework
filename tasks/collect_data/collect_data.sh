...
for ROBOT in ur5e panda; do
    ...
    python collect_task.py ${SAVE_DIR} --num_workers ${NUM_WORKERS} --ctrl_config ${CTRL_CONFIG} --N ${NUM} --per_task_group ${PER_TASK} --task_name ${TASK_NAME} --robot ${ROBOT} --overwrite --collect_cam --n_env ${N_env} --gpu_id_indx ${GPU_ID_INDX} --object_set ${OBJECT_SET}
done 