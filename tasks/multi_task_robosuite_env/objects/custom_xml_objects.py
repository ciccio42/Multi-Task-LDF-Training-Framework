...
from multi_task_robosuite_env.objects.meta_xml_objects import MyMujocoXMLObject
...
class NamedRoundNutObject(MujocoXMLObject):
    """
    Round nut (used in NutAssembly)
    """

    def __init__(self, name):
        super().__init__(os.path.join(BASE_DIR, f"{name}.xml"),
                         name=name, joints=[
                             dict(type="free", damping="0.0005")],
                         obj_type="all", duplicate_collision_geoms=True)
...
class BoxObject(MujocoXMLObject):
    def __init__(self, name="greenbox"):
        super().__init__(os.path.join(BASE_DIR, f'{name}.xml'), name)
