...
class WipeArena(TableArena):
    ...
    def __init__(
        ...
        self.target_dirt = target_dirt_id
    ...
    def configure_location(self):
        ...
        material_list = [grayplaster, redwood, dirt]
        for i in range(3):
            pos = self.sample_start_pos()
            for j in range(self.num_markers):
                # If we're using two clusters, we resample the starting position and direction at the halfway point
                if self.two_clusters and j == int(np.floor(self.num_markers / 2)):
                    pos = self.sample_start_pos()

                marker_name = f'contact{i}{j}'
                ...
                if i == self.target_dirt:
                    self.markers.append(marker)
                else:
                    self.distractor[i].append(marker)
    ...
    def reset_arena(self, sim):
        ...
        for i in range(3):
            pos = self.sample_start_pos()
            if i == self.target_dirt:
                marker_lst = self.markers
            else:
                marker_lst = self.distractor[i]
            # Loop through all visual markers
            for j, marker in enumerate(marker_lst):
                # If we're using two clusters, we resample the starting position and direction at the halfway point
                if self.two_clusters and j == int(np.floor(self.num_markers / 2)):
                    pos = self.sample_start_pos()
