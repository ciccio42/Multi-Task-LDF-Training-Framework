...
class TransformerNetwork(nn.Module):
    ...
    def __init__(
        ...
        if type(input_tensor_space) == DictConfig and type(output_tensor_space) == DictConfig: # if we pass conf via hydra
            self._input_tensor_space, self._output_tensor_space = self.define_spaces(input_tensor_space, output_tensor_space)
        else:
    ...
    def define_spaces(self, input_tensor_cfg, output_tensor_cfg):
        
        image_conf = input_tensor_cfg['image']
        nle_conf = input_tensor_cfg['natural_language_embedding']
        
        state_space = spaces.Dict(
        {
            'image': spaces.Box(
                            low=image_conf['low'],
                            high=image_conf['high'], 
                            shape=image_conf['shape'],
                            dtype=eval(image_conf['dtype'])),
            'natural_language_embedding': spaces.Box(
                                                low=eval(nle_conf['low']),
                                                high=eval(nle_conf['high']), 
                                                shape=nle_conf['shape'],
                                                dtype=eval(nle_conf['dtype']))
        }
        )
    ...
    def forward(self,
        ...
        if outer_rank == 1:  # This is an inference call
            ...
            action_logits = output_tokens[:, predicted_action_index] # (bs, t*tokens_per_action, vocab_size->256)
    ...
    def _get_tokens_and_mask(self,
        ...
        # print_mask = self._default_attention_mask.tolist()
    ...
    def _tokenize_images(self, observations, network_state):
        ...
        context_image_tokens = self._image_tokenizer(image, context=context) # efficientNet + tokenlearner (batch, t, num_tokens, embedding_dim)
